...
namespace Microsoft.Samples.VisualStudio.ComboBox
{
	...
	public sealed class ComboBoxPackage : Package
	{
		...
		private const string defaultItemName = "TestSettings";
		...
		private string GetCurrentDropDownComboChoice()
		{
			...
			{
				InitConfigurations();
				_currentDropDownComboChoice = Environment.GetEnvironmentVariable(envName, EnvironmentVariableTarget.Machine) ?? _dropDownComboChoices.FirstOrDefault() ?? "No config";
			}
		...
		private string KillTestRunners()
		{
			var runners = System.Diagnostics.Process.GetProcesses().Where(s => s.ProcessName.Contains("vstest")
																			   || s.ProcessName.Contains("mstest")
																			   || s.ProcessName.Contains("xunit")
			);
		...
		{
			string runners = KillTestRunners();

			String envValue = currentDropDownComboChoice == defaultItemName ? String.Empty : currentDropDownComboChoice;
